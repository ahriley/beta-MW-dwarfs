...
def lnlike(theta, data, data_covs):
    shifts = data - theta[:3]
    sig_r, sig_theta, sig_phi = theta[3:6]
...
def lnprior(theta):
    ...
    s = theta[3:6]
    corr = theta[6:]
    if ((m<500).all() and (m>-500).all() and (s<300).all() and
        (s>0).all() and (corr<=1).all() and (corr>=-1).all()):
        return 0.0
    return -np.inf
...
def sample_prior(nwalkers):
    scale = np.array([1000,1000,1000,300,300,300,2,2,2])
    shift = np.array([500,500,500,0,0,0,1,1,1])
    ndim = len(scale)
